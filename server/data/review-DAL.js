    ...
    async function removeReview(id) {
        await mongoose.connect(uri);
        const removedItem = await UserReview.findByIdAndUpdate(id, { 'isDeleted': true }, { new: true });
        await mongoose.disconnect();
        return removedItem;
    }
    ...
    async function deleteReview(reviewID) {
        await mongoose.connect(uri);
        const result = await UserReview.findByIdAndUpdate(reviewID, {'isDeleted': true}, { new: true }).exec();
        await mongoose.disconnect();
        return result !== null;
    }
    ...
    await mongoose.connect(uri);
    const result = await UserReview.findByIdAndUpdate(reviewID, { 'isDeleted': false }, { new: true }).exec();
    await mongoose.disconnect();
    return result !== null;
    }
